...
async function collect(
  ...
  try {
    // Prove the watcher is DELIVERING before doing anything that depends on it.
    //
    // `watchWorkingTree` returns synchronously, but `fs.watch` underneath is
    // backed by FSEvents on macOS and inotify on Linux, and neither starts
    // delivering the moment the object exists. Alone, the gap is invisible.
    // Under load — the runner executes test files in parallel — it widens past
    // the write, and the event for that write is never emitted at all.
    //
    // The expectation tests then fail after their full 10s poll, having seen
    // nothing. The ABSENCE tests are worse: they pass, because a watcher that
    // is not listening reports nothing, which is exactly what they assert. A
    // test that cannot distinguish "correctly silent" from "not running" is
    // not testing anything.
    //
    // So: write a probe, wait until its event arrives, then forget it. From
    // that point the watcher is known live, and both kinds of assertion mean
    // what they say.
    // The probe is written REPEATEDLY, not once. A single write before the
    // watcher goes live produces no event at all, so polling for it waits out
    // the whole deadline for something that will never arrive — which is how
    // the first version of this barrier failed, and is the same shape as the
    // bug it exists to catch. Each attempt changes the contents, so the
    // digest guard cannot collapse it into the previous one.
    const probe = '__watch-probe.md';
    const ready = Date.now() + 10_000;
    let attempt = 0;
    while (!seen.some(c => c.path === probe) && Date.now() < ready) {
      await writeFile(join(folder, probe), `probe ${attempt++}\n`);
      // Scaled to the window actually in use, not the module default. A probe
      // rewritten faster than the debounce is overwritten before it can ever
      // settle into a report, so the barrier starves exactly when a caller
      // widens the window.
      await new Promise(r => setTimeout(r, debounceMs * 2));
    }
    assert.ok(
      seen.some(c => c.path === probe),
      'the watcher never reported the probe file, so it was never listening',
    );
    seen.length = 0;
...
test('an atomic save (temp write + rename) collapses to ONE report of the final file', async () => {
  ...
  await withFolder(async folder => {
    ...
    const seen = await collect(folder, async () => {
      ...
      // Same reasoning as the rapid-writes test: an atomic save is a burst
      // (temp write, then rename), and collapsing a burst is only a guarantee
      // within one window. Under load the burst outgrows the default and two
      // reports become correct, which turns this into a measurement of the
      // machine (#305).
    }, seen => seen.some(c => c.path === 'doc.md'), DEBOUNCE * 6);
...
test('rapid successive writes collapse to one report with the final contents', async () => {
  await withFolder(async folder => {
    // A wider window than the default, deliberately. The guarantee under test
    // is "writes WITHIN one debounce window collapse to one report" — so the
    // window has to dominate the burst for the test to be testing that. Under
    // load four writes can take longer than the default window, at which point
    // two reports is correct behaviour and the assertion is measuring the
    // machine rather than the watcher.
    const seen = await collect(folder, async () => {
      const f = join(folder, 'fast.md');
      for (const v of ['a', 'b', 'c', 'final']) await writeFile(f, `${v}\n`);
    }, seen => seen.some(c => c.path === 'fast.md'), DEBOUNCE * 6);
...
// A file that did not exist when watching began (#N).
//
// Every other test here edits a file the folder already had. The app's own
// "New File" path creates one in an open workspace, which is a different case:
// the watcher has no prior digest for the path, the settle loop's first stat
// is the file's first appearance, and nothing has ever reported it.
//
// Reported symptom: a new file lands in the workspace folder and shows under
// Open Files, while the workspace row still says "No documents" — the log
// never learns about it.
test('a file created after watching starts is reported', async () => {
  await withFolder(async folder => {
    const seen = await collect(
      folder,
      async () => {
        await new Promise(r => setTimeout(r, DEBOUNCE * 2));
        await writeFile(join(folder, 'Test Plaintext File.txt'), 'Test file\n');
      },
      seen => seen.some(c => c.path === 'Test Plaintext File.txt'),
    );
    const hit = seen.find(c => c.path === 'Test Plaintext File.txt');
    assert.ok(hit, `no event for a newly created file; saw ${JSON.stringify(seen.map(s => s.path))}`);
    assert.equal(text(hit.bytes), 'Test file\n');
  });
});
