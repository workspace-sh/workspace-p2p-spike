...
export class WorkspaceChild {
  ...
  /** The DHT used when a create or open request names none. */
  private defaultBootstrap: BootstrapNode[] | undefined;

  constructor(write: (s: string) => void, options: { bootstrap?: BootstrapNode[] } = {}) {
    this.write = write;
    this.defaultBootstrap = options.bootstrap;
  }

  /**
   * Close every workspace this child holds, each flushing its log into its
   * folder. For the parent going away without closing them: an app that quits.
   * A workspace that fails to close does not stop the others.
   */
  async closeAll(): Promise<void> {
    for (const stop of this.watchers.values()) stop();
    this.watchers.clear();
    const open = [...this.workspaces.values()];
    this.workspaces.clear();
    await Promise.all(open.map((ws) => ws.close().catch(() => {})));
  }
    ...
    switch (params.method) {
      case 'wsCreate': {
        const ws = await Workspace.create({
          ...
          identitySeed: params.identitySeedHex
            ...
            : undefined,
          bootstrap: params.bootstrap ?? this.defaultBootstrap,
        });
      ...
      case 'wsOpen': {
        const ws = await Workspace.open({
          ...
          ...(params.rootSeedHex !== undefined ? { rootSeed: hexToBytes(params.rootSeedHex) } : {}),
          bootstrap: params.bootstrap ?? this.defaultBootstrap,
        });
        return this.track(ws);
      }
      ...
      case 'wsStatus': {
        return this.require(params.handle).status() satisfies WsStatusResult;
      }
      case 'wsClose': {
        const ws = this.require(params.handle);
  ...
  private track(ws: Workspace): WsInfoResult {
    ...
    ws.onStatus((status) => {
      const evt: StatusEvent = { event: 'status', handle, ...status };
      this.send(evt);
    });
    return {
      handle,
