...
export type Event = ChangeEvent | WorkingTreeEvent | StatusEvent;
...
/** A workspace's peers, announce or writability changed. */
export interface StatusEvent {
  event: 'status';
  handle: string;
  peers: number;
  announce: 'pending' | 'announced' | 'failed' | 'local-only';
  writable: boolean;
}
...
export type Params =
  ...
  | {
      ...
      folder: string;
      identitySeedHex: Hex;
      /** The workspace's root seed, from the device that created it. Makes the handle an admin. */
      rootSeedHex?: Hex;
      storage: string;
      bootstrap?: BootstrapNode[];
    }
...
export interface WsInfoResult {
  ...
  rootDid: string;
  isAdmin: boolean;
  /** Whether this device can append to the data log. False for an invited member. */
  writable: boolean;
  length: number;
}
export type WsStatusResult = Omit<StatusEvent, 'event' | 'handle'>;
