...
export class RemoteWorkspace {
  ...
  /** True if this device can append to the data log; false for an invited member. */
  ...
  private statusListeners = new Set<(status: WsStatusResult) => void>();
  ...
  private constructor(transport: Transport, info: WsInfoResult, ownsTransport: boolean) {
    ...
    // A child that predates the field could always write, so absence is true.
    this.writable = info.writable !== false;
  ...
  /** Peers, announce and writability, as the child sees them now. */
  async status(): Promise<WsStatusResult> {
    return (await this.rpc({ method: 'wsStatus', handle: this.handle })) as WsStatusResult;
  }

  /**
   * Hear when status changes. Returns an unsubscribe. A change can land
   * before a caller subscribes, so read `status()` once after subscribing.
   */
  onStatus(cb: (status: WsStatusResult) => void): () => void {
  ...
  private handleMessage(msg: unknown): void {
    ...
    if ('event' in msg) {
      const evt = msg as ChangeEvent | WorkingTreeEvent | StatusEvent;
      if (evt.event === 'status' && evt.handle === this.handle) {
        const { event: _event, handle: _handle, ...status } = evt;
