...
export class NodeRuntime implements P2PRuntime {
  ...
  /** Connections replicating now. Lifetime: each until it closes. */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly peers = new Set<any>();
  private readonly peerListeners = new Set<(count: number) => void>();
  ...
  /**
   * Count `conn` as a peer until it closes.
   *
   * Counted at the point replication starts, so the number means "peers this
   * device is exchanging blocks with" and a connection refused at the gate
   * never appears in it.
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private admit(conn: any): void {
    if (this.peers.has(conn)) return;
    this.peers.add(conn);
    this.notifyPeers();
    const forget = () => {
      if (this.peers.delete(conn)) this.notifyPeers();
    };
    if (typeof conn.once === 'function') conn.once('close', forget);
  }

  private notifyPeers(): void {
    const count = this.peers.size;
    for (const cb of this.peerListeners) cb(count);
  }
  ...
  onPeersChanged(cb: (count: number) => void): () => void {
