...
export interface P2PRuntime {
  ...
  /**
   * How many peers this runtime is replicating with now: connections that
   * passed the membership gate, or every connection on an ungated swarm.
   * Optional; absent means the runtime does not count.
   */
  peerCount?(): number;

  /** Hear when `peerCount` changes. Returns an unsubscribe. Optional, as above. */
